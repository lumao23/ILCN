...
class HICOEvaluator():
    ...
    def evaluation_default(self):
        ...
        self.APRcurve(self.pre, self.rec, self.mark)
        ...
        mAP['mean_max_recall'] = np.asarray(max_recall).mean()
        mAP['mAP'] = map_ / count
        ...
        return {"mAP_def": mAP['mAP'], "mAP_def_rare": mAP['mAP_rare'], "mAP_def_non_rare": mAP['mAP_non_rare'],
                "mean_max_recall": mAP['mean_max_recall']}
    ...
    def eval_hoi(self, hoi_id, global_ids, gt_dets, pred_anno,
        ...
        self.pre.append(np.asarray(precision))
        self.rec.append(np.asarray(recall))
    ...
    def APRcurve(self, precisions, recalls, marks):
        assert len(precisions) == 600
        ...
        for t in x:
            ...
            for i in range(len(precisions)):
                rec = recalls[i].copy()
                pre = precisions[i].copy()
                if not marks[i] or pre[0] == 0:
                    continue
                if np.sum(rec >= t) == 0:
                    continue
                else:
                    p.append(np.max(pre[rec >= t]))
                    r.append(np.min(rec[rec >= t]))
            if len(p) == 0:
                break
            p = np.asarray(p).sum()/600
            r = np.asarray(r).mean()
            prec.append(p)
            reca.append(r)

        with open("QAhoi.txt","a") as f:
            for j in range(len(prec)):
                data = [prec[j], reca[j], x[j]]
                f.write(str(prec[j])+","+str(reca[j])+","+str(x[j])+"\n")
